Welcome to the ``murefi`` documentation!
========================================

.. image:: https://img.shields.io/badge/code%20on-Github-lightgrey
    :target: https://github.com/laurahelleckes/murefi

``murefi`` is a Python package for **mu**\ lti **re**\ plicate **fi**\ tting, mostly for building ordinary differential equation (ODE) models.

You can download the latest version from `GitHub <https://github.com/laurahelleckes/murefi>`_ or install it via ``pip install murefi``.

In the following chapters, we introduce the data structures, basic concepts and modeling API.

.. toctree::
   :maxdepth: 2
   :numbered:
   :caption: Contents

   01_datastructures
   02_models
   03_fitting

Learn from the examples how a ``murefi``-driven modeling workflow looks like:

.. toctree::
   :maxdepth: 2
   :caption: Examples

   Example_01_Datastructures
   Example_02_ParameterMapping
   Example_03_ModelFitting


You can find autogenerated API documentation below:

.. toctree::
   :maxdepth: 2
   :caption: API Reference

   murefi_datastructures
   murefi_core
   murefi_ode
   murefi_symbolic
   murefi_objectives
   murefi_contrib
